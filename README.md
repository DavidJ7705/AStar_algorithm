...
Pathfinding is one of those problems that sounds simple until you actually try to solve it. A* sits at an interesting intersection - it's grounded in real computer science theory, but it also forces you to make concrete engineering decisions about how you structure your code, manage memory, and handle edge cases.

At its core, A* combines the reliability of Dijkstra's algorithm with the directional instinct of Greedy Best-First Search. Dijkstra guarantees the shortest path by exhaustively exploring all directions, but pays for that with speed. Greedy Best-First moves fast by always chasing the goal, but can miss the optimal route. A* balances both - it tracks the actual cost already travelled *and* an estimated cost to the goal, meaning it stays efficient without sacrificing correctness, provided the heuristic never overestimates.

The goal of this project was to implement A* in modern C++ across a grid-based environment with configurable obstacles, a start position, and a goal position. But the more interesting challenge was in the design - building a clean, modular system using modern C++ features like smart pointers, `std::priority_queue`, `std::mt19937`, and `std::chrono`, while keeping responsibilities clearly separated across classes. The project grew iteratively across lab sessions, starting from a basic 2D vector grid and gradually evolving into a multi-file system with multiple heuristics, exception handling, and deliberate architectural decisions at each step.
...
![old_calculate_heuristic](images/old_calculate_heuristic.png)
...
![new_calculate_heuristic](images/new_calculate_heuristic.png)
...
![main code](images/main_code.png)
...
![raw_pointer_node](images/raw_pointer_node.png)


// New - fixed address, automatic cleanup

![smart_pointer_node](images/smart_pointer_node.png)


### `std::priority_queue`

The original open list used a `std::vector<Node>` with a manual for-loop to find the lowest f score on every iteration - O(n) per step. 


![old open list](images/old_openlist.png)


Replacing it with `std::priority_queue` and a `CompareNode` comparator struct means the lowest f node is always at the top, retrieved instantly with `top()`. The ordering is maintained automatically on every `push`.

![new open list](images/new_openlist.png)
...
![old enum class](images/old_enum_class.png)



// Modern - scoped and type-safe

![new enum class](images/new_enum_class.png)
...
![friend for point](images/friend_for_point.png)
...
Old approach:
![old for loops](images/old_for_loops.png)
...
![new for loops](images/new_for_loops.png)
